...
|--------------|------------|-------------------------|
| `naive`      | **seasonal random walk** — forecast for horizon *h* is the newest lagged level a whole number of years before the forecast quarter (`level_{(4−h mod 4) mod 4}`: h=1 ← `level_3`, h=2 ← `level_2`, h=3 ← `level_1`, h=4 ← `level_0`, repeating). Same-fiscal-quarter alignment; the paper's baseline anchor. Every model worth shipping beats it. | measured: naive+fade together **17 min / 11 GB peak**, writing two full-coverage 550,620,720-row forecasts (~3.5 GB each) |
